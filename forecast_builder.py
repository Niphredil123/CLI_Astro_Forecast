...
def sun_forecast_build(dates: list, user_lat: float, user_lng: float) -> dict:
    ...
    logging.debug(f'sun_forecast is {sun_forecast}')
    return sun_forecast
...
def vc_forecast_build(vc_api_key: str,
    ...
    if vc_api_key != 'xxx':
        ...
        if lunar_api_response is not None:
            ...
            logging.debug(f'lunar_forecast is: {lunar_forecast}')
            ...
            logging.debug(f'cloud_forecast is: {cloud_forecast}')
...
def aurora_forecast_build(dates: list,
    ...
    logging.debug(f'aurora_prob is: {aurora_prob}')
    # Running the aurora_api_call function to call the API and receive the
    ...
    logging.debug(f'aurora_3day is: {aurora_3day}')
